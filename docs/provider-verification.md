...
| Z.ai / GLM | [Z.ai API reference](https://docs.z.ai/api-reference/introduction.md) — OpenAI-compatible base `https://api.z.ai/api/paas/v4`; China mirror `https://open.bigmodel.cn/api/paas/v4` | `GET /models` for key validation on both `zai` and `glm`. No documented balance or quota API; subscription (GLM Coding Plan) and PAYG both directed to `z.ai/manage-apikey/billing`. Key: `ZAI_API_KEY`; fallbacks: `GLM_API_KEY`, `ZHIPU_API_KEY`. |
